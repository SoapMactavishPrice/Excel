...
export default class LwcProductIntrestedPage extends NavigationMixin(LightningElement) {
    ...
    lookupRecord(event) {
        ...
        //console.log('pbe', JSON.stringify(selectedCampaign));
        //const pbe = event.detail.selectedRecords;
        //console.log('pbe', pbe);
        //if (this.addAnswer && this.addAnswer[index]) {
        // this.addAnswer[index] = { ...this.addAnswer[index], prodId: pbe.proId,pbeId:pbe.Id, prodName: pbe.Name, prodCode: pbe.productCode,unitPrice:pbe.unitPrice, prodFamily: pbe.familyField };  // This ensures the view updates
        console.log('OUTPUT : ', selectedRecord.proId);
        console.log('OUTPUT : ', selectedRecord.Id);
        console.log('OUTPUT : ', selectedRecord.Name);
        console.log('OUTPUT : ', selectedRecord.productCode);
        console.log('OUTPUT : ', selectedRecord.unitPrice);
        console.log('OUTPUT : ', selectedRecord.familyField);
    ...
    getProd() {
        ...
        getProdInterest({ Id: this.recordId }).then(result => {
            ...
            if (data.currencyCode == null || data.currencyCode == '' || data.currencyCode == undefined) {
                ...
                if (tempData.length > 0) {
                    tempData.forEach((ele, index) => {
                        let temp = {
                            ...
                            //    Add_In_Opty: true,
                            Add_In_Opty: ele.Add_In_Opty__c !== false,
                            New_Product: ele.New_Product__c,
                            New_Product_Name: ele.New_Product_Name__c,
                            prodFamily: ele.Product_Family__c,
                            isEdit: true,
                            frequency: ele.Volume_Frequency__c,
                            pbeId: ele.Pricebook_Entry_Id__c,
                            unitPrice: parseFloat(ele.List_Price__c),
    ...
    handleCheckboxChange(event) {
        ...
        this.addAnswer[index] = {
            ...
            Add_In_Opty: isChecked ? false : this.addAnswer[index].Add_In_Opty,
    ...
    handleSelectChange(event) {
        ...
        if (label === 'Add_In_Opty') {
            // Only allow changing Add_In_Opty if New_Product is false
            if (!this.addAnswer[index].New_Product) {
                this.addAnswer[index].Add_In_Opty = checked;
                // Force UI update
                this.addAnswer = [...this.addAnswer];
            } else {
                // If New_Product is true, revert the checkbox to false
                event.target.checked = false;
                this.addAnswer[index].Add_In_Opty = false;
            }
        } else {
