...
import getAssessmentMaster from '@salesforce/apex/LwcStageGateScoreControllerStage5.getAssessmentMaster';
import getAssessmentAnswer from '@salesforce/apex/LwcStageGateScoreControllerStage5.getAssessmentAnswer';
import getDescisionCriteria from '@salesforce/apex/LwcStageGateScoreControllerStage5.getDescisionCriteria';
import saveData from '@salesforce/apex/LwcStageGateScoreControllerStage5.saveData';
import getFiledDisplay from '@salesforce/apex/LwcStageGateScoreControllerStage5.getFiledDisplay';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import { NavigationMixin } from 'lightning/navigation';
import { getRecord } from 'lightning/uiRecordApi';
const FIELDS = ['Milestone__c.Final_Outcome__c'];
import getDocumentUrl from '@salesforce/apex/LwcStageGateScoreControllerStage5.getDocumentUrl';
import deletefile from '@salesforce/apex/LwcStageGateScoreControllerStage5.deletefile';
import USER_ID from '@salesforce/user/Id';  // Importing the logged-in user's ID
import getRecordDetail from '@salesforce/apex/LwcStageGateScoreControllerStage5.getRecordDetail';


export default class LwcStageGateScoreStageGate5 extends NavigationMixin(LightningElement) {
    @api recordId;
    @track total = 0;
...
this.total = this.assData.reduce((sum, item) => {
    const name = item?.Assessment_Answer__r?.Name;
...
@track showReason  = false;
    ...
    handleSave() {
        let tempCheck = true;
        console.log('OUTPUT : ',this.openDecisionCriteria);
        ...
        if (tempCheck) {
            ...
            saveData({ JS: JSON.stringify(this.assData), descJs: JSON.stringify(this.decisionData), score: this.total, recordId: this.recordId, label: this.label
            ,label: this.label,approverLabel:this.selectedValue,openDecisionCriteria:this.openDecisionCriteria,reason:this.rejectedReson }).then(result => {
                ...
                if (result.Message && result.Message.startsWith('Success')) {
                    ...
                    this.showToast('Success', `Launch Stage Record Updated successfully.`, 'Success');
                    this.handleCancel();
